...
> **Phiên bản tài liệu:** 5.2.0-ARCHITECTURE-SPLIT  
...
| **2026-09-12** | `5.2.0-ARCHITECTURE-SPLIT` | **Tái cấu trúc Kiến trúc 5 Giai đoạn (Modular Architecture, Cloud Functions, Dynamic Pharmacopoeia & Security Hardening)**: Hoàn thành toàn diện 5 giai đoạn hiện đại hóa nền tảng PQM. [Phase 1: Phân rã God Files] Tách `aiTools.ts` (1,790 dòng) thành 17 tool modules độc lập trong `src/services/ai/tools/` với gateway/barrel router; Phân tách `ProductDetail.tsx` (1,163 dòng) và `BatchList.tsx` (998 dòng) thành cấu trúc thư mục chức năng với custom hooks `useProductDetail`, `useBatchList` và các micro-components (`ProductHeader`, `ProductMetricsCard`, `ProductInfoTab`, `ProductFormulaTab`, `ProductTccsTab`, `ProductHistoryTab`, `ProductAnalyticsTab`, `BatchGridItem`, `BatchListItem`, `BatchStatusSelect`, `batchProgress`). Duy trì 100% backward compatibility qua re-export gateways. [Phase 2: Giải tỏa Fat Client qua Firebase Cloud Functions] Thiết lập thư mục `functions/` với Node.js/TypeScript backend; xây dựng 4 Cloud Functions: `calculateSPCMetrics` (tính Cp, Cpk, Pp, Ppk và 8 quy tắc Nelson trên server), `autoHealConsistencyCron` (cron-job 2h sáng tự động quét và hàn gắn CSDL), `generateQualityReport` (tạo Excel bằng SheetJS trên backend, lưu Firebase Storage và trả Signed URL), `onUserRoleChanged` (DB trigger tự động gán Custom Claims); xây dựng client service `cloudFunctionsService.ts` với cơ chế Hybrid Fallback mượt mà khi offline. [Phase 3: Động hóa Cấu hình Dược điển (Dynamic Configuration)] Di dời cơ sở dữ liệu dược điển cứng lên Firebase RTDB node `pharmacopoeia_standards/` với 31 tiêu chuẩn mặc định; xây dựng module quản trị trực quan `PharmacopoeiaManager.tsx` trong `SettingsPage` cho phép QA CRUD động các tiêu chuẩn DĐVN, USP, BP và nạp dữ liệu mẫu; kết nối Dynamic Context cho AI Copilot (`lookupPharmacopoeiaDynamic`). [Phase 4: Tái thiết kế Security Rules & Custom Claims] Nâng cấp `database.rules.json` sử dụng trực tiếp Token Claims (`auth.token.role`, `auth.token.isAdmin`) thay vì lookup CSDL nặng nề, giảm độ trễ và tăng tính bảo mật, đồng thời bảo toàn tương thích ngược cho các bài audit tĩnh. [Phase 5: Củng cố Testing & CI/CD] Bổ sung unit tests cho sub-components và services mới (`useProductDetail.test.tsx`, `useBatchList.test.tsx`, `batchProgress.test.ts`, `pharmacopoeiaService.test.ts`, `cloudFunctionsService.test.ts`); toàn bộ 65 test suites (407 bài tests) Vitest passed 100%; `npm run build` hoàn thành với 0 lỗi. | AI Pair Programmer |
| **2026-09-12** | `5.1.2-FULL-ADMIN-RIGHTS` | **Khôi phục & Trao Quyền Toàn diện Tối cao cho Admin (Full Admin Capability Hardening)**: Khắc phục triệt để lỗi "Từ chối quyền: Bạn không có quyền lập phiếu kiểm nghiệm mới" khi Quản trị viên (Admin) tạo hoặc cập nhật phiếu kiểm nghiệm và các thực thể khác. [1. Đồng bộ Danh tính Người dùng & Quyền Admin] Sửa lỗi object `state.user` trong Zustand bị mất thuộc tính `role` và `isAdmin` do nhận trực tiếp từ Firebase Auth; nâng cấp `setUser`, `setIsAdmin`, `setRole` trong `authSlice.ts` và `useAuthSync.ts` để luôn đồng bộ trạng thái `role: 'ADMIN'` và `isAdmin: true` vào `state.user`. [2. Bộ Phân Giải Danh Tính Trung Tâm (`resolveCurrentIdentity`)] Xây dựng hàm `resolveCurrentIdentity` trong `storeHelpers.ts` và áp dụng vào toàn bộ các Slices (`testResultSlice`, `batchSlice`, `productSlice`, `tccsSlice`), đảm bảo 100% các cuộc gọi sang Application Services đều mang danh tính chuẩn xác. [3. Fallback Cấp Hệ Thống & Đa Tầng Nhận Diện] Bổ sung fallback kiểm tra `window.__PQM_GET_CURRENT_USER__` trong `normalizeUser` (`permissionService.ts`), nhận diện vai trò Admin ngay cả khi tham số truyền vào là raw Firebase user. [4. Toàn Quyền Cho Tất Cả Vai Trò] Cập nhật `hasRole` trong `permissionService.ts` để Quản trị viên tự động thỏa mãn mọi vai trò chuyên môn (`QA`, `QC`, `LAB`, `PRODUCTION`). Đạt 60/60 suites (393/393 tests) passed 100%, `tsc --noEmit` và `npm run build` hoàn thành với 0 lỗi. | AI Pair Programmer |
| **2026-09-12** | `5.1.1-AI-SCAN-FIX` | **Tối ưu hóa Luồng AI Quét Phiếu Kết Quả & Chuyển Form Nhập Mẫu (AI OCR & Form Transfer Hardening)**: Khắc phục triệt để các lỗi khi quét OCR phiếu kết quả và chuyển dữ liệu sang Form nhập mẫu (`/test-results/new`). [1. Phân bổ Chỉ tiêu Thông minh] Khắc phục tình trạng toàn bộ chỉ tiêu bị chuyển sang "Chỉ tiêu bổ sung": tự động phân giải danh mục chỉ tiêu TCCS từ Lô đã khớp (`matchedBatch`), TCCS theo ngày sản xuất, hoặc TCCS hiệu lực của sản phẩm trước khi phân loại; nạp chính xác các chỉ tiêu đã nhận diện vào bảng TCCS chính (`testResultsMap`). [2. Tự Động Tạo Lô Nâng Cấp] Sửa lỗi tạo lô mới trong form và chat: sinh UUID tự động (`generateId('batch')`), gán `status: TESTING`, liên kết `tccsId` hiệu lực của sản phẩm, tự động chuyển đổi định dạng ngày ISO (`parseDateToISO`), và di dời tức thì các chỉ tiêu khớp vào bảng đánh giá chính sau khi tạo lô. [3. Chuẩn Hóa Quản Lý Dữ Liệu Nháp AI] Bổ sung `batchId` vào `NormalizedAIData` và `normalizeAIData`; giữ lại tên chỉ tiêu gốc cho các chỉ tiêu chưa map thay vì để trống khiến dữ liệu bị lọc bỏ. [4. Tối ưu UX Điều hướng Chat] Khi xác nhận modal ánh xạ chỉ tiêu trong Chatbot AI (`MappingConfirmModal`), lập tức lưu bản nháp AI và điều hướng trực tiếp sang Form nhập kết quả kiểm nghiệm (`/test-results/new`). [5. Tìm Kiếm Lô An Toàn] Bổ sung kiểm tra an toàn chống crash khi trường `batchNo` của lô rỗng hoặc undefined. Đạt 60/60 suites (390/390 tests) passed 100%, `tsc --noEmit` và `npm run build` hoàn thành với 0 lỗi. | AI Pair Programmer |
