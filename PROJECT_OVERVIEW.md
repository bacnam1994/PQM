...
| **2026-08-21** | `1.2.4` | **Triển khai gói nâng cấp toàn diện theo Upgrade Suggestions**: <br/>- **Audit Log UI (Nhật ký kiểm toán)**: Tạo [AuditLogPage.tsx](file:///d:/26%20Kiem%20nghiem/PQM/src/pages/system/AuditLogPage.tsx) và nâng cấp [auditService.ts](file:///d:/26%20Kiem%20nghiem/PQM/src/services/auditService.ts) hỗ trợ xem, lọc theo hành động/phân hệ/người dùng, phân trang và xuất file CSV báo cáo kiểm toán theo chuẩn GMP.<br/>- **Cảnh báo chất lượng Real-time**: Kết nối `useQualityAlerts` vào [Layout.tsx](file:///d:/26%20Kiem%20nghiem/PQM/src/components/layout/Layout.tsx), hiển thị Badge số lượng cảnh báo động trên Sidebar và nút chuông thông báo Topbar kèm animation nổi bật.<br/>- **Dark Mode toàn diện cho AlertsPage**: Nâng cấp [AlertsPage.tsx](file:///d:/26%20Kiem%20nghiem/PQM/src/pages/quality/AlertsPage.tsx) tương thích 100% chế độ ban đêm với tông màu thiết kế chuyên nghiệp.<br/>- **Trang Public QR CoA Verification**: Tạo [CoAVerifyPage.tsx](file:///d:/26%20Kiem%20nghiem/PQM/src/pages/public/CoAVerifyPage.tsx) cho phép khách hàng, đối tác quét mã QR trên CoA in ra để xác thực chứng chỉ chất lượng chính hãng không cần đăng nhập.<br/>- **Đạt 48/48 tests passed 100%** trong Vitest với unit test mới cho [auditService.test.ts](file:///d:/26%20Kiem%20nghiem/PQM/src/services/auditService.test.ts). | AI Pair Programmer |
| **2026-08-21** | `1.2.3` | **Tự động áp dụng giới hạn mặc định $\pm$20% cho chỉ tiêu không có trong TCCS**: <br/>- Tích hợp tính toán giới hạn mặc định 80% ~ 120% hàm lượng công bố khi chỉ tiêu kiểm nghiệm thuộc công thức nhưng không có trong TCCS.<br/>- Bổ sung nhóm Amino Acids và Hoạt chất Supplement phổ biến (L-Lysine, L-Arginine, CoQ10, Collagen, Omega-3...) vào `PHARMA_TERM_DICTIONARY` ([aiMapping.ts](file:///d:/26%20Kiem%20nghiem/PQM/src/utils/aiMapping.ts)).<br/>- Bổ sung Lớp 4 Dice Coefficient trong `lookupFormulaItem` ([CoAReport.tsx](file:///d:/26%20Kiem%20nghiem/PQM/src/components/features/CoAReport.tsx)). | AI Pair Programmer |
| **2026-08-21** | `1.2.2` | **CoA tự động tính % cho Chỉ tiêu có trong Công thức nhưng KHÔNG có trong TCCS**: <br/>- Thêm hàm `lookupFormulaItem()` (3-layer cascade) vào [CoAReport.tsx](file:///d:/26%20Kiem%20nghiem/PQM/src/components/features/CoAReport.tsx): **Lớp 1** Exact match tên normalize → **Lớp 2** `PHARMA_TERM_DICTIONARY` (canonical name, VD: "Kẽm (Zn)" ↔ "Kẽm gluconat") → **Lớp 3** `isCriteriaMatch()` fuzzy semantic.<br/>- Khi tìm được thành phần công thức, CoA hiển thị `% so với declaredContent` trong cột Kết quả, cột Yêu cầu hiển thị *"Theo công thức"* (chữ nghiêng).<br/>- Thêm sentinel `__FORMULA__` trong `getLimitText()` + `renderLimitCell()` để render JSX riêng biệt cho cột Yêu cầu mà không ảnh hưởng các hàm xử lý chuỗi khác.<br/>- Import `lookupPharmaTerm` và `isCriteriaMatch` từ [aiMapping.ts](file:///d:/26%20Kiem%20nghiem/PQM/src/utils/aiMapping.ts) — tái sử dụng hạ tầng AI có sẵn, 0 duplicate code.<br/>- Build TypeScript 0 errors. | AI Pair Programmer |
