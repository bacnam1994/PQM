...
import { BookUser, Plus, Search, Loader2, Save, Tag, Layers3, X as XIcon, AlertTriangle } from 'lucide-react';
...
const RawMaterialCatalog: React.FC = () => {
  ...
  const handleDelete = async () => {
    if (crud.selectedItem) {
      // KIỂM TRA: Nguyên liệu có đang được sử dụng trong công thức sản phẩm không
      try {
        // Query tất cả công thức sản phẩm để kiểm tra
        const formulasRef = ref(db, 'product_formulas');
        const formulasSnap = await get(formulasRef);
        
        if (formulasSnap.exists()) {
          const formulas = formulasSnap.val();
          ...
          Object.values(formulas).forEach((formula: any) => {
            ...
            if (formula.ingredients && Array.isArray(formula.ingredients)) {
              formula.ingredients.forEach((ing: any) => {
                if (ing.materialId === crud.selectedItem?.id) {
                  formulasUsingMaterial.push(`Công thức ID: ${formula.id}`);
                }
              });
            }
            // Kiểm tra trong excipients
            if (formula.excipients && Array.isArray(formula.excipients)) {
              formula.excipients.forEach((exc: any) => {
                if (exc.materialId === crud.selectedItem?.id) {
                  formulasUsingMaterial.push(`Công thức ID: ${formula.id}`);
                }
              });
            }
          ...
          // Nếu nguyên liệu đang được sử dụng, hiển thị cảnh báo và không cho xóa
          if (formulasUsingMaterial.length > 0) {
            notify({ 
              type: 'ERROR', 
              title: 'Không thể xóa', 
              message: `Nguyên liệu này đang được sử dụng trong ${formulasUsingMaterial.length} công thức sản phẩm. Vui lòng xóa hoặc cập nhật các công thức liên quan trước.` 
            });
            crud.close();
            return;
          }
        ...
        console.error("Lỗi kiểm tra khi xóa nguyên liệu:", error);
        notify({ type: 'ERROR', message: 'Lỗi kiểm tra dữ liệu. Vui lòng thử lại.' });
      }
    }
  };
