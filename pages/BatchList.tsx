...
const BatchGridItem = memo(({ batch, isExpanded, onExpand, onEdit, onDelete, onView, testResults, onUpdateBatchStatus, isAdmin }: {
  ...
  return (
    <DSCard isExpanded={isExpanded} className={`p-6 relative group ${isExpanded ? 'col-span-2' : ''}`}>
      {/* Cảnh báo hạn dùng */}
      {isExpired && <div className="absolute top-0 right-0 bg-red-500 text-white text-[9px] font-black px-3 py-1 rounded-bl-xl uppercase tracking-widest flex items-center gap-1"><CalendarOff size={10}/> Đã hết hạn</div>}
      {isNearExpiry && <div className="absolute top-0 right-0 bg-amber-500 text-white text-[9px] font-black px-3 py-1 rounded-bl-xl uppercase tracking-widest flex items-center gap-1"><AlertTriangle size={10}/> Cận date ({diffDays} ngày)</div>}

      <div className="flex justify-between items-start mb-4">
        <div className="p-3 bg-slate-50 rounded-2xl text-indigo-600"><Hash size={24}/></div>
        <BatchStatusSelect 
          status={batch.status} 
          batchId={batch.id} 
      ...
      {isExpanded && (
        <div className="mt-4 pt-4 border-t border-slate-100 animate-in fade-in">
          <div className="space-y-2">
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2"><FlaskConical size={12}/> Tiến độ kiểm nghiệm ({progressPercent}%)</h4>
            ...
            {missingCriteria.length > 0 ? (
              <div className="bg-amber-50 p-3 rounded-xl border border-amber-100">
                <p className="text-[10px] font-bold text-amber-700 mb-1 flex items-center gap-1"><ListChecks size={12}/> Còn thiếu {missingCriteria.length} chỉ tiêu:</p>
                <div className="flex flex-wrap gap-1">
                  {missingCriteria.map((c, idx) => (
                    <span key={idx} className="px-2 py-0.5 bg-white border border-amber-200 text-amber-800 text-[9px] font-bold rounded-md">{c.name}</span>
                  ))}
                </div>
              </div>
            ) : (
  ...
  return (
    <tr className="hover:bg-slate-50 transition-colors">
      ...
      <td className="px-4 py-3 text-center text-xs">
        <div className="text-slate-500">Cỡ lô: <span className="font-bold">{batch.theoreticalYield?.toLocaleString()}</span></div>
        <div className="text-slate-700">Sản lượng: <span className="font-bold">{batch.actualYield?.toLocaleString()}</span></div>
...
const BatchList: React.FC = () => {
  ...
  const handleUpdateBatch = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Loại bỏ các trường hydrated (product, tccs) trước khi lưu
    const { product, tccs, ...cleanBatch } = crud.selectedItem as any;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <PageHeader 
        ...
        action={
          <div className="flex gap-3">
            ...
            <AddButton onClick={() => { setSelectedProductId(''); setProductSearch(''); crud.openAdd(); }} label="Đăng ký Lô mới" />
          </div>
        } 
      />
      ...
      <DSFilterBar>
        ...
        <DSSelect value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-32">
      ...
      {isAdvancedFilterOpen && (
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-4 animate-in slide-in-from-top-2">
           <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1"><CalendarRange size={12}/> Từ ngày (SX)</label>
              <input type="date" value={dateRange.from} onChange={e => setDateRange({...dateRange, from: e.target.value})} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500" />
           </div>
           <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1"><CalendarRange size={12}/> Đến ngày (SX)</label>
              <input type="date" value={dateRange.to} onChange={e => setDateRange({...dateRange, to: e.target.value})} className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500" />
           </div>
           <div className="space-y-1 md:col-span-2">
              <div className="flex justify-between items-center">
                <label className="text-[10px] font-bold text-slate-400 uppercase">Sản phẩm cụ thể</label>
                <button onClick={() => { setDateRange({ from: '', to: '' }); setFilterProductId(''); setFilterStatus('ALL'); }} className="text-[10px] font-bold text-red-500 hover:underline flex items-center gap-1"><X size={10}/> Xóa bộ lọc</button>
              </div>
              <select 
      ...
      <Modal isOpen={crud.mode === 'ADD'} onClose={crud.close} title="Đăng ký Lô Sản xuất" icon={Plus}>
        <form onSubmit={handleAddBatch} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-4">
            <button type="button" onClick={crud.close} className="px-6 py-3 text-slate-400 font-black uppercase text-xs">Hủy</button>
      ...
      <Modal isOpen={crud.mode === 'EDIT'} onClose={crud.close} title="Cập nhật thông tin Lô" icon={Edit2}>
        {crud.selectedItem && (
          <form onSubmit={handleUpdateBatch} className="space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-4">
              <button type="button" onClick={crud.close} className="px-6 py-3 text-slate-400 font-black uppercase text-xs">Hủy</button>
              <button type="submit" disabled={isSubmitting} className="px-10 py-3 bg-blue-600 text-white rounded-xl font-black uppercase text-xs disabled:opacity-70 flex items-center gap-2">
                {isSubmitting && <Loader2 size={14} className="animate-spin" />}
                Cập nhật
              </button>
            </div>
      ...
      <Modal isOpen={isImportModalOpen} onClose={() => setIsImportModalOpen(false)} title="Nhập dữ liệu hàng loạt" icon={FileSpreadsheet} color="bg-indigo-600">
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <textarea value={importText} onChange={(e) => setImportText(e.target.value)} rows={8} className="w-full p-6 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2rem] font-mono text-xs focus:ring-4 focus:ring-indigo-50 outline-none transition-all" placeholder="Copy từ Excel và dán tại đây..." />
          </div>
      ...
      >
        {viewBatch && (
           <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                 <div className="flex justify-between items-start mb-4">
                    <div>
                       ...
                       <h4 className="font-bold text-slate-800 text-lg">{(viewBatch as any).product?.name}</h4>
                       <p className="text-xs font-bold text-indigo-600">{(viewBatch as any).product?.code}</p>
                    </div>
                 ...
                 <div className="grid grid-cols-2 gap-4 text-xs">
                    <div><span className="font-bold text-slate-500">NSX:</span> {formatDateStandard(viewBatch.mfgDate)}</div>
              ...
              <div>
                 ...
                 <div className="space-y-4">
                    {testResults.filter(r => r.batchId === viewBatch.id).length === 0 ? (
                        ...
                        testResults.filter(r => r.batchId === viewBatch.id).map(res => (
                            <div key={res.id} className="border border-slate-200 rounded-xl overflow-hidden">
                                <div className="bg-slate-50 px-4 py-3 flex justify-between items-center border-b border-slate-200">
                                    <div className="flex items-center gap-3">
                                        <span className={`px-2 py-1 rounded text-[9px] font-black uppercase ${res.overallStatus === 'PASS' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
              ...
              <div className="flex justify-end"><button onClick={() => setViewBatch(null)} className="px-6 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg text-xs font-bold uppercase transition-colors">Đóng</button></div>
           </div>
        )}
      </Modal>
    </div>
