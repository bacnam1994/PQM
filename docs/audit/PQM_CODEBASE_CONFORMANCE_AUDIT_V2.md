...
> **CẬP NHẬT TIẾN TRÌNH: HOÀN TẤT STEP 1A (SPECIFICATION REMEDIATION & FREEZE) & STEP 1B (AUDIT RE-CHECK)**:  
> Sau khi phát hiện 42 Broken References và 14 Specification Conflicts trong lần quét đầu, quy trình chuẩn hóa đặc tả **STEP 1A** đã được thực thi và đóng băng thành công mà **KHÔNG ĐỤNG ĐẾN MÃ NGUỒN NGHIỆP VỤ (`src/`)**.  
> Kết quả tái kiểm toán **STEP 1B** xác nhận: **Hệ thống đặc tả đã đạt trạng thái 100% SẠCH & KHÉP KÍN (Zero Phantom References, Zero Spec Conflicts)**.

### Bảng Chỉ Số Đo Lường Sau Khi Hoàn Tất STEP 1A & STEP 1B:

| Hạng mục đo lường                                           | Ban đầu (Step 1) | Sau Chuẩn hóa (Step 1A/1B) | Trạng thái Nghiệm thu | Ghi chú & Đánh giá rủi ro                                                                                                                                                                            |
| :---------------------------------------------------------- | :--------------: | :------------------------: | :-------------------: | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Tổng số Specification Documents**                         |     **108**      |          **108**           |       `FROZEN`        | Khóa Single Source of Truth (SSoT) 10 bộ quy tắc `BR_01`..`BR_10`; chuyển các file cũ vào `reference/`.                                                                                              |
| **Tổng số Liên kết Gãy Đặc tả (Broken References)**         |      **42**      |           **0**            |     ✅ **CLEAN**      | Đã sửa toàn bộ RTM `PQM_TRACEABILITY_MATRIX_V2.md`, trỏ 100% vào 61 file code và test thực tế đang tồn tại trong repository.                                                                         |
| **Tổng số Xung đột Đặc tả Nội tại (Spec Conflicts)**        |      **14**      |           **0**            |     ✅ **CLEAN**      | Đã giải quyết triệt để: Khóa 7 Release Gates (thay vì 5); Đồng bộ 25 mã màn hình `SC-01`..`SC-25`; Tách 3 tầng trạng thái độc lập; Đóng băng Alternate Rule V2 (`FAIL_RETRY` & `CONDITIONAL_CHECK`). |
| **Số lượng SSoT Trùng lặp (Duplicate SSoT)**                |      **2**       |           **0**            |     ✅ **CLEAN**      | `docs/business-rules/README.md` khẳng định duy nhất bộ `BR_01` đến `BR_10`; xóa bỏ hoàn toàn xung đột giữa các file lẻ un-prefixed.                                                                  |
| **Màn hình chưa có hợp đồng (Unmapped Screens)**            |      **2**       |           **0**            |     ✅ **CLEAN**      | 25 màn hình định danh trong `SCREEN_INVENTORY.md` đã khớp 1:1 với `SCREEN_CONTRACTS.md` (SC-01 đến SC-25).                                                                                           |
| **Tổng số Source Code Files (`src/`)**                      |     **501**      |          **501**           |      `INSPECTED`      | Chưa can thiệp refactor (bảo toàn nguyên vẹn cho Step 2).                                                                                                                                            |
| **Tổng số Test Suites Đang Chạy Thành Công**                |     **138**      |          **138**           |    ✅ `100% PASS`     | Chạy `npx vitest run`: **138 test files passed (1,292 tests passed)**, 0 failures.                                                                                                                   |
| **Tổng số Xung đột Mã nguồn cần sửa (Code Conflicts)**      |      **8**       |           **8**            |    `READY_STEP_2`     | Mismatch FSM trong `stateMachine.ts`, CoA client re-evaluation trong `CoAReport.tsx`, Hardcoded Gate 5 & Gate 6 trong `ReleaseRules.ts`. Đây là mục tiêu chính của STEP 2.                           |
| **Tổng số Logic nghiệp vụ Trùng lặp (Duplicated Code)**     |      **8**       |           **8**            |    `READY_STEP_2`     | Sẽ được gộp vào Single Domain Service / Resolver trong Step 2.                                                                                                                                       |
| **Tổng số Triển khai Còn Thiếu (Missing Implementation)**   |      **11**      |           **11**           |    `READY_STEP_2`     | Hash Chain ALCOA+, Batch 9-state FSM, TestResult decoupled FSM, Router OOS/CAPA. Sẽ được triển khai trong Step 2.                                                                                    |
| **Tổng số Thành phần Cũ Cần Xử Lý (Legacy Implementation)** |      **19**      |           **19**           |    `READY_STEP_2`     | Loại bỏ boolean `isPass`, xóa bỏ magic string status trong Step 2.                                                                                                                                   |

---

## B. Specification Consistency Matrix (Ma Trận Nhất Quán Giữa Các Tài Liệu Đặc Tả - STEP 1B RE-CHECK)

Bảng đối chiếu tính nhất quán nội tại giữa các tài liệu đặc tả kỹ thuật trong thư mục `docs/` sau khi thực thi **STEP 1A Remediation**:

|  STT   | Tài liệu Nguồn (Source)                                   | Tài liệu Đích (Target)                        |   Trạng thái    | Vấn đề phát hiện ban đầu (Problem)                                                                  | Kết quả khắc phục STEP 1A & STEP 1B (Remediation Result)                                                                                                 |
| :----: | :-------------------------------------------------------- | :-------------------------------------------- | :-------------: | :-------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | --------- | ------------------ |
| **1**  | `docs/traceability/PQM_TRACEABILITY_MATRIX_V2.md`         | `docs/functional-specs/`                      | ✅ **RESOLVED** | Tham chiếu phantom `FRS_01_MASTER_DATA.md`... không tồn tại.                                        | Đã chuẩn hóa: Trỏ trực tiếp vào `PQM_FUNCTIONAL_REQUIREMENTS_V2.md` kèm Section/FRS ID thực tế.                                                          |
| **2**  | `docs/traceability/PQM_TRACEABILITY_MATRIX_V2.md`         | `src/` (Code Implementation)                  | ✅ **RESOLVED** | Tham chiếu 18 file mã nguồn giả định không tồn tại trên đĩa.                                        | Đã sửa toàn bộ cột `[6]` trỏ chính xác vào các Service hiện hữu (`src/services/app/...`, `src/domain/...`). Quét xác thực 100% file tồn tại.             |
| **3**  | `docs/traceability/PQM_TRACEABILITY_MATRIX_V2.md`         | `tests/` (Automated Tests)                    | ✅ **RESOLVED** | Tham chiếu danh sách file test không tồn tại.                                                       | Đã cập nhật cột `[7]` trỏ vào các test suite thực tế đang chạy pass 100% trong `tests/` và `src/`.                                                       |
| **4**  | `docs/acceptance/TRACEABILITY_MATRIX_V2.md`               | `tests/` & `src/`                             | ✅ **RESOLVED** | Mã Release Gate ghi "5 Trụ cột", lệch so với 7 Gates của Master Workflow.                           | Đã chuẩn hóa thành "Chốt chặn Xuất xưởng 7 Cổng (7 Release Gates)" đồng bộ toàn diện với Master Workflow.                                                |
| **5**  | `docs/business-rules/` (Bộ 10 file `BR_01` -> `BR_10`)    | `docs/business-rules/` (Bộ 21 file lẻ)        | ✅ **RESOLVED** | Tồn tại 2 bộ quy tắc gây nhầm lẫn Single Source of Truth (SSoT).                                    | Đã chuyển 21 file lẻ vào `docs/business-rules/reference/`, công bố bộ 10 file `BR_01`..`BR_10` là SSoT duy nhất tại `docs/business-rules/README.md`.     |
| **6**  | `docs/screen-contracts/SCREEN_INVENTORY.md`               | `docs/contracts/SCREEN_CONTRACTS.md`          | ✅ **RESOLVED** | Lệch mã màn hình (SC-10 = Batch Detail vs Batch List, SC-14 = CoA vs PKN).                          | Đã viết lại `SCREEN_CONTRACTS.md` khớp 1:1 với `SCREEN_INVENTORY.md` (SC-01 đến SC-25 chuẩn tắc).                                                        |
| **7**  | `docs/functional-specs/PQM_FUNCTIONAL_REQUIREMENTS_V2.md` | `docs/workflow/PQM_SYSTEM_WORKFLOW_MASTER.md` | ✅ **RESOLVED** | `FRS-REL-001` ghi 5 điều kiện trong khi Master Workflow là 7 Release Gates.                         | Đã cập nhật `FRS-REL-001` định nghĩa chi tiết đầy đủ 7 Release Gates theo đúng Hiến pháp Master Workflow.                                                |
| **8**  | `docs/business-rules/BR_06_ALTERNATE_RULES.md`            | `docs/contracts/ALTERNATE_RULE_CONTRACT.md`   | ✅ **RESOLVED** | Xung đột Main PASS -> Alt là `EXEMPTED` vs `NOT_APPLICABLE`; Scope chưa đóng.                       | Đã chuẩn hóa: Main PASS -> Alt là `NOT_APPLICABLE`. Đóng băng phạm vi V2 chỉ hỗ trợ `FAIL_RETRY` & `CONDITIONAL_CHECK`; gắn cờ FUTURE cho các loại khác. |
| **9**  | `docs/contracts/STATE_MACHINES.md` (FSM 3)                | `docs/contracts/QUALITY_STATUS_CONTRACT.md`   | ✅ **RESOLVED** | FSM 3 trộn lẫn trạng thái thực thi (`REQUIRED`, `TESTING`) với kết quả chất lượng (`PASS`, `FAIL`). | Đã tách FSM 3 thành: FSM 3A (`CriterionExecutionState`) và Pure Quality Resolver (`CriterionQualityStatus`), triệt tiêu xung đột ngữ nghĩa.              |
| **10** | `docs/contracts/CRITERION_STATE_CONTRACT.md`              | `docs/contracts/QUALITY_STATUS_CONTRACT.md`   | ✅ **RESOLVED** | Enum `CriterionQualityStatus` chứa `NOT_EVALUATED` gây trùng lặp với `PENDING`.                     | Đã chuẩn hóa: `CriterionQualityStatus = 'PASS'                                                                                                           | 'FAIL' | 'PENDING' | 'NOT_APPLICABLE'`. |
