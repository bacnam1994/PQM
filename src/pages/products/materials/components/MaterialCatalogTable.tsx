...
import { DSCard, DSTable, ActionButtons, VirtualizedTableBody } from '../../../../components';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {paginatedItems.map((material) => {
            ...
            return (
              <div
                key={material.id}
                className="p-4 rounded-xl flex flex-col justify-between hover:border-emerald-500/30 transition-all duration-200 group relative overflow-hidden bg-surface border border-border shadow-xs"
              >
                <div>
                    ...
                    {material.aliases && material.aliases.length > 0 ? (
                      <div className="flex flex-wrap gap-1">
                        {material.aliases.slice(0, 3).map((al, aIdx) => (
                          <span
                            key={aIdx}
                            className="bg-surface-2 text-ink-soft border border-border px-2 py-0.5 rounded-full text-[11px] font-medium"
                          >
                            {al}
                          </span>
                        ))}
                ...
                <div className="mt-3.5 pt-2.5 border-t border-border/80 flex items-center justify-between">
                  <div className="flex items-center gap-1">
                    {usedProducts.length > 0 ? (
                      ...
                      <span className="text-xs text-ink-muted italic">Chưa dùng</span>
                    )}
                  </div>
                ...
                return (
                  <tr key={material.id} className="hover:bg-surface-2/60 transition-colors">
                    <td className="px-4 py-3 font-mono font-medium text-xs text-ink whitespace-nowrap">
                      {material.code || (
                        <span className="text-ink-muted font-normal italic">Chưa gán</span>
                      )}
                    </td>
      ...
      {filteredCatalogLength === 0 && (
        <div className="p-12 text-center bg-surface rounded-xl border border-border">
          ...
          <p className="text-xs text-ink-muted mt-1">
            Hãy thử thay đổi từ khóa tìm kiếm hoặc bấm nút "Thêm Nguyên liệu" để tạo mới.
          </p>
        </div>
      )}
    </>
