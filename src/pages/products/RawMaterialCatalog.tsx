        ...
        <tbody className="divide-y divide-slate-50 dark:divide-zinc-850">
          {currentItems.map(material => (
            <tr key={material.id} className="hover:bg-slate-50 dark:hover:bg-zinc-900/40 transition-colors">
              <td className="px-4 py-3 font-bold text-slate-700 dark:text-zinc-200 text-sm">{material.name}</td>
              <td className="px-4 py-3">
                {material.aliases && material.aliases.length > 0 ? (
                  <div className="flex flex-wrap gap-2 items-center">
                    <span className="bg-indigo-100 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-400 px-2 py-0.5 rounded text-xs font-bold">{material.aliases.length} tên</span>
                    <span className="text-slate-400 dark:text-zinc-500 text-xs truncate hidden xl:inline" title={material.aliases.join(', ')}>
