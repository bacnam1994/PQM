...
const ProductFormPage = () => {
  ...
  const uploadToFirebaseStorage = async (file: File): Promise<string> => {
    try {
      ...
      return await uploadStorageFile(file, path, (progress) => {
      ...
      notify({
        type: 'ERROR',
        message: 'Lỗi khi tải file lên Storage: ' + (error?.message || 'Lỗi không xác định'),
      });
      throw error;
    }
  };
    ...
    try {
      ...
      if (useGoogleDriveUpload && googleDriveClientId && googleDriveApiKey) {
        ...
        try {
          ...
          notify({
            type: 'WARNING',
            message: 'Lỗi tải lên Google Drive. Đang chuyển sang lưu Firebase Storage...',
          });
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!data.code && !data.name) {
        notify({
          type: 'WARNING',
          message: 'Vui lòng nhập tối thiểu Mã sản phẩm hoặc Tên sản phẩm!',
        });
      ...
      const isDuplicate = products.some(
        (p) => p.id !== id && p.code === data.code && p.name === data.name
      );
      if (isDuplicate) {
        notify({
          type: 'ERROR',
          title: 'Trùng lặp',
          message: 'Sản phẩm với cùng Mã và Tên đã tồn tại!',
        });
        setIsSubmitting(false);
        return;
      }
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-ink-muted mt-0.5">
            {id
              ? 'Cập nhật danh mục và hồ sơ kỹ thuật sản phẩm'
              : 'Đăng ký sản phẩm thương mại mới vào hệ thống'}
          </p>
        </div>
      </div>
          ...
          <form onSubmit={handleSave} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <DSFormInput
                ...
                placeholder="VD: VB-001"
                required
              />
              <DSFormInput
                label="Nhóm hàng"
                name="group"
                defaultValue={productToEdit?.group}
                placeholder="VD: TPBS / Mỹ phẩm..."
              />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <DSFormInput
                label="Số ĐKCB / Công bố"
                name="registrationNo"
                defaultValue={productToEdit?.registrationNo}
                placeholder="VD: 1234/2024/ATTP-XNCB"
              />
              <DSFormInput
                type="date"
                label="Ngày cấp"
                ...
                defaultValue={
                  productToEdit?.registrationDate?.split('T')[0] ||
            ...
            <DSFormInput
              label="Công ty đăng ký / Sở hữu"
              name="registrant"
              defaultValue={productToEdit?.registrant}
                ...
                {imageUrl ? (
                  <div className="relative group shrink-0">
                    <img
                      src={imageUrl}
                      alt="Preview"
                      className="w-24 h-24 rounded-xl object-cover border border-border shadow-xs"
                    />
                    <button
                      type="button"
                      onClick={() => setImageUrl('')}
