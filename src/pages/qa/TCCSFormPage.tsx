...
const validateTCCS = (values: typeof initialTccsFormState) => {
  ...
  const zodResult = tccsFormSchema.safeParse({
    ...
    packaging: (values as any).packaging || '',
    storage: (values as any).storage || '',
    shelfLife: (values as any).shelfLife || '',
...
const TCCSFormPage = () => {
      ...
      if (tccs) {
        ...
        setValues({
          ...
          issueDate: cloneId
            ...
            : parseDateToISO(tccs.issueDate),
          mainCriteria: (tccs.mainQualityCriteria || []).map((c) => ({
            ...c,
  ...
  const handleApplyPharmacopoeiaTemplate = (dosageForm: DosageFormType) => {
    ...
    template.criteria.forEach((item) => {
      const exists = formValues.mainCriteria.some(
  ...
  const handleFetchCriteriaFromFormula = (defaultTolerancePercent: number = 10) => {
    ...
    criteria.forEach((crit) => {
      const exists = formValues.mainCriteria.some(
        (c) => c.name.toLowerCase() === crit.name.toLowerCase()
      );
      if (!exists) {
          ...
          <div className="space-y-6 pr-1">
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                {errors.productId && (
                  <p className="text-red-500 text-xs font-medium mt-1 pl-1">{errors.productId}</p>
                )}
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  />
                  {errors.code && (
                    <p className="text-red-500 text-xs font-medium mt-1 pl-1">{errors.code}</p>
                  )}
                </div>
                <div className="space-y-1 flex items-end">
                  <DSDateInput
                    value={formValues.issueDate}
                    onChange={(val) => setFieldValue('issueDate', val)}
                  />
                  {errors.issueDate && (
                    <p className="text-red-500 text-xs font-medium mt-1 pl-1">{errors.issueDate}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              {formValues.productId &&
                selectedFormula &&
                checkTCCSFormulaConflicts(formValues.mainCriteria, selectedFormula).length > 0 && (
                  <div className="p-3 bg-amber-50/80 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/40 rounded-xl space-y-1">
                    <p className="text-xs font-bold text-amber-800 dark:text-amber-300 flex items-center gap-1.5">
                      <ExclamationCircleIcon className="h-4 w-4 text-amber-600" /> Cảnh báo đồng bộ
                      Công thức & TCCS:
                    </p>
                    <ul className="text-xs font-medium text-amber-700 dark:text-amber-400 space-y-0.5 pl-4 list-disc">
                      {checkTCCSFormulaConflicts(formValues.mainCriteria, selectedFormula).map(
                        (warn, wIdx) => (
                          <li key={wIdx}>{warn}</li>
                        )
                      )}
              ...
              {formValues.mainCriteria.map((c, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`flex flex-col gap-1 p-2 rounded-xl border transition-all group ${isMinMaxError ? 'bg-rose-500/10 border-rose-500/30' : 'bg-surface-2 border-border hover:border-border-strong'}`}
                  >
                    <div className="flex gap-2 items-center">
                      <select
                        value={c.type}
                        onChange={(e) =>
                          updateInArray('mainCriteria', i, 'type', e.target.value as any)
                        }
                      ...
                      <input
                        ...
                        value={c.name}
                        onChange={(e) => updateInArray('mainCriteria', i, 'name', e.target.value)}
                        ...
                        list="criteria-name-suggestions"
                      />
                      <input
                        placeholder="ĐVT"
                        value={c.unit}
                        onChange={(e) => updateInArray('mainCriteria', i, 'unit', e.target.value)}
                        ...
                        list="criteria-unit-suggestions"
                      />
                      {c.type === CriterionType.NUMBER ? (
                        <>
                          <input
                            ...
                            onBlur={(e) => {
                              ...
                              if (v !== '' && !isNaN(n) && !v.trim().endsWith('.'))
                                updateInArray('mainCriteria', i, 'min', n);
                            }}
                            className={`w-20 px-3 py-2 bg-surface text-ink rounded-lg text-xs font-semibold outline-none text-right border border-border shadow-xs font-mono ${isMinMaxError ? 'border-rose-500 text-rose-600 bg-rose-500/10' : ''}`}
                          />
                          <input
                            type="text"
                            inputMode="decimal"
                            ...
                            onBlur={(e) => {
                              ...
                              const n = parseNumberFromText(v);
                              if (v !== '' && !isNaN(n) && !v.trim().endsWith('.'))
                                updateInArray('mainCriteria', i, 'max', n);
                            }}
                            className={`w-20 px-3 py-2 bg-surface text-ink rounded-lg text-xs font-semibold outline-none text-right border border-border shadow-xs font-mono ${isMinMaxError ? 'border-rose-500 text-rose-600 bg-rose-500/10' : ''}`}
                          />
                          <input
                            type="text"
                            placeholder="HL công bố"
                            ...
                            className="w-24 px-3 py-2 bg-emerald-500/10 rounded-lg text-xs font-semibold outline-none text-right border border-emerald-500/20 shadow-xs font-mono text-emerald-700 dark:text-emerald-400"
                            title="Hàm lượng công bố (để làm gốc tính %)"
                          />
                        </>
                      ) : (
                        <div className="flex-[2] flex gap-2">
                          <div className="flex-1 flex flex-col gap-1">
                            <input
                              type="text"
                              placeholder="Mức quy định (VD: 15 ± 20%...)"
                              value={c.expectedText || ''}
                              onChange={(e) =>
                                updateInArray('mainCriteria', i, 'expectedText', e.target.value)
                              }
                              className="w-full px-3 py-2 bg-surface text-ink placeholder:text-ink-muted rounded-lg text-xs font-semibold outline-none border border-border shadow-xs"
                            />
                            {c.expectedText && calculateRangePreview(c.expectedText) && (
                              <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold pl-1">
                                {calculateRangePreview(c.expectedText)}
                              </span>
                            )}
                          </div>
                          <input
                      ...
                      <button
                        type="button"
                        onClick={() => removeFromArray('mainCriteria', i)}
                    ...
                    <div className="flex items-center gap-2 px-2 opacity-60 group-hover:opacity-100 transition-opacity">
                      ...
                      <input
                        placeholder="Ghi chú / Điều kiện (VD: Phương pháp thử...)"
                    ...
                    <div className="flex items-center gap-2 px-2 mt-1 opacity-70 group-hover:opacity-100 transition-opacity">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          updateInArray('mainCriteria', i, 'formulaIngredientId', e.target.value)
                        }
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      addToArray('microbiologicalCriteria', {
                        name: '',
                        unit: '',
                        max: undefined,
                        type: CriterionType.NUMBER,
                    ...
                    className="p-2 bg-rose-500/10 text-rose-600 dark:text-rose-400 rounded-lg hover:bg-rose-500/20 transition-colors"
                  >
                ...
                {formValues.microbiologicalCriteria.map((c, i) => (
                  ...
                  >
                    <select
                      value={c.type}
                      onChange={(e) =>
                        updateInArray('microbiologicalCriteria', i, 'type', e.target.value as any)
                      }
                      className="w-16 px-1 py-2 bg-surface text-ink rounded-lg text-xs font-semibold outline-none border border-border shadow-xs"
                    >
                      <option value="NUMBER">Số</option>
                    ...
                    <input
                      ...
                      value={c.name}
                      onChange={(e) =>
                        updateInArray('microbiologicalCriteria', i, 'name', e.target.value)
                      }
                      className="flex-[2] px-3 py-2 bg-surface text-ink placeholder:text-ink-muted rounded-lg text-xs font-semibold outline-none border border-border shadow-xs"
                      list="criteria-name-suggestions"
                    />
                    <input
                      ...
                      value={c.unit}
                      onChange={(e) =>
                        updateInArray('microbiologicalCriteria', i, 'unit', e.target.value)
                      }
                      className="w-16 px-3 py-2 bg-surface text-ink placeholder:text-ink-muted rounded-lg text-xs font-semibold outline-none text-center border border-border shadow-xs"
                      list="criteria-unit-suggestions"
                    />
                      ...
                      <div className="flex items-center gap-2 bg-surface px-3 w-32 border border-border shadow-xs rounded-lg">
                        ...
                        <input
                          ...
                          onBlur={(e) => {
                            ...
                            const n = parseNumberFromText(v);
                            if (v !== '' && !isNaN(n) && !v.trim().endsWith('.'))
                              updateInArray('microbiologicalCriteria', i, 'max', n);
                          }}
                    ...
                    <button
                      type="button"
                      onClick={() => removeFromArray('microbiologicalCriteria', i)}
                      className="p-2 text-ink-muted hover:text-rose-500 transition-colors"
                    >
                      <XMarkIcon className="w-4 h-4" />
                    </button>
                ...
                {formValues.heavyMetalCriteria.map((c, i) => (
                  ...
                  >
                    <select
                      value={c.type}
                      onChange={(e) =>
                        updateInArray('heavyMetalCriteria', i, 'type', e.target.value as any)
                      }
                      className="w-16 px-1 py-2 bg-surface text-ink rounded-lg text-xs font-semibold outline-none border border-border shadow-xs"
                    >
                      <option value="NUMBER">Số</option>
                    ...
                    <input
                      ...
                      value={c.name}
                      onChange={(e) =>
                        updateInArray('heavyMetalCriteria', i, 'name', e.target.value)
                      }
                      className="flex-[2] px-3 py-2 bg-surface text-ink placeholder:text-ink-muted rounded-lg text-xs font-semibold outline-none border border-border shadow-xs"
                      list="criteria-name-suggestions"
                    />
                    <input
                      ...
                      value={c.unit}
                      onChange={(e) =>
                        updateInArray('heavyMetalCriteria', i, 'unit', e.target.value)
                      }
                      className="w-16 px-3 py-2 bg-surface text-ink placeholder:text-ink-muted rounded-lg text-xs font-semibold outline-none text-center border border-border shadow-xs"
                      list="criteria-unit-suggestions"
                    />
                    {c.type === CriterionType.NUMBER ? (
                      <div className="flex items-center gap-2 bg-surface px-3 w-32 border border-border shadow-xs rounded-lg">
                        ...
                        <input
                          ...
                          onBlur={(e) => {
                            ...
                            const n = parseNumberFromText(v);
                            if (v !== '' && !isNaN(n) && !v.trim().endsWith('.'))
                              updateInArray('heavyMetalCriteria', i, 'max', n);
                          }}
                          className="w-full bg-transparent py-2 text-xs font-semibold text-ink outline-none text-right font-mono"
                        />
                      </div>
                      ...
                      <input
                        ...
                        value={c.expectedText || ''}
                        onChange={(e) =>
                          updateInArray('heavyMetalCriteria', i, 'expectedText', e.target.value)
                        }
                        className="flex-[2] px-3 py-2 bg-surface text-ink placeholder:text-ink-muted rounded-lg text-xs font-semibold outline-none border border-border shadow-xs"
                      />
                    )}
                    <button
                      type="button"
                      onClick={() => removeFromArray('heavyMetalCriteria', i)}
                      className="p-2 text-ink-muted hover:text-rose-500 transition-colors"
                    >
                      <XMarkIcon className="w-4 h-4" />
                    </button>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="p-2 bg-amber-500/10 text-amber-600 dark:text-amber-400 rounded-lg hover:bg-amber-500/20 transition-colors"
                  >
                    <PlusIcon className="w-4 h-4" />
                  </button>
                </div>
                {(formValues.mycotoxinCriteria || []).length === 0 ? (
                  <p className="text-xs text-ink-muted italic bg-surface-2 p-3 rounded-xl border border-border">
                    Chưa có chỉ tiêu độc tố vi nấm / dư lượng nào. Nhấn dấu (+) để thêm nếu sản phẩm
                    yêu cầu.
                  </p>
                ) : (
                  (formValues.mycotoxinCriteria || []).map((c, i) => (
                    ...
                    >
                      <select
                        value={c.type}
                        onChange={(e) =>
                          updateInArray('mycotoxinCriteria', i, 'type', e.target.value as any)
                        }
                        className="w-16 px-1 py-2 bg-surface text-ink rounded-lg text-xs font-semibold outline-none border border-border shadow-xs"
                      >
                        <option value="NUMBER">Số</option>
                      ...
                      <input
                        placeholder="Tên chỉ tiêu (VD: Aflatoxin B1...)"
                        value={c.name}
                        onChange={(e) =>
                          updateInArray('mycotoxinCriteria', i, 'name', e.target.value)
                        }
                        className="flex-[2] px-3 py-2 bg-surface text-ink placeholder:text-ink-muted rounded-lg text-xs font-semibold outline-none border border-border shadow-xs"
                        list="criteria-name-suggestions"
                      />
                      <input
                        ...
                        value={c.unit}
                        onChange={(e) =>
                          updateInArray('mycotoxinCriteria', i, 'unit', e.target.value)
                        }
                        className="w-16 px-3 py-2 bg-surface text-ink placeholder:text-ink-muted rounded-lg text-xs font-semibold outline-none text-center border border-border shadow-xs"
                        list="criteria-unit-suggestions"
                      />
                      {c.type === CriterionType.NUMBER ? (
                        <div className="flex items-center gap-2 bg-surface px-3 w-32 border border-border shadow-xs rounded-lg">
                          ...
                          <input
                            ...
                            onBlur={(e) => {
                              ...
                              const n = parseNumberFromText(v);
                              if (v !== '' && !isNaN(n) && !v.trim().endsWith('.'))
                                updateInArray('mycotoxinCriteria', i, 'max', n);
                            }}
                            className="w-full bg-transparent py-2 text-xs font-semibold text-ink outline-none text-right font-mono"
                          />
                        </div>
                        ...
                        <input
                          ...
                          value={c.expectedText || ''}
                          onChange={(e) =>
                            updateInArray('mycotoxinCriteria', i, 'expectedText', e.target.value)
                          }
                          className="flex-[2] px-3 py-2 bg-surface text-ink placeholder:text-ink-muted rounded-lg text-xs font-semibold outline-none border border-border shadow-xs"
                        />
                      )}
                      <button
                        type="button"
                        onClick={() => removeFromArray('mycotoxinCriteria', i)}
                        className="p-2 text-ink-muted hover:text-rose-500 transition-colors"
                      >
                        <XMarkIcon className="w-4 h-4" />
                      </button>
            ...
            <div className="space-y-3 pt-4 border-t border-border">
              <div className="flex justify-between items-center">
                ...
                <button
                  type="button"
                  onClick={() => addToArray('alternateRules', { main: '', alt: '' })}
              ...
              {formValues.alternateRules.map((rule, i) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => removeFromArray('alternateRules', i)}
                      className="p-2 text-ink-muted hover:text-rose-500 transition-colors"
                    >
                      <XMarkIcon className="w-4 h-4" />
                    </button>
                  ...
                  <div className="flex items-center gap-2 pl-2">
                    <select
                      ...
                      onChange={(e) =>
                        updateInArray('alternateRules', i, 'type', e.target.value as any)
                      }
                    ...
                    {rule.type === 'CONDITIONAL_CHECK' && (
                      <input
                        ...
                        onChange={(e) =>
                          updateInArray('alternateRules', i, 'conditionValue', e.target.value)
                        }
