...
interface FormulaExcipientsTableProps {
  excipients: FormulaIngredient[];
  rawMaterials: RawMaterial[];
  materialMap: Map<string, RawMaterial>;
  onAddExcipient: () => void;
  onChangeExcipient: (index: number, field: keyof FormulaIngredient, value: any) => void;
  onSelectMaterial: (index: number, material: RawMaterial) => void;
  onQuickCreateMaterial: (index: number, name: string) => Promise<void>;
  onRemoveExcipient: (index: number) => void;
}

export const FormulaExcipientsTable: React.FC<FormulaExcipientsTableProps> = ({
  excipients,
  rawMaterials,
  materialMap,
  onAddExcipient,
          ...
          const matchingMaterials = rawMaterials.filter(
            (m) =>
              ...
              (Array.isArray(m.aliases) &&
                m.aliases.some((a) => normalizeSearch(a).includes(normalizeSearch(exc.name))))
          );
          ...
          return (
            ...
            >
              <div className="col-span-7 relative">
                <div className="relative flex items-center">
                  <input
                    ...
                    value={exc.name}
                    onChange={(e) => {
                      onChangeExcipient(index, 'name', e.target.value);
                      ...
                      {exc.name.trim() &&
                        !matchingMaterials.some(
                          ...
                          <div
                            onMouseDown={(e) => e.preventDefault()}
                            onClick={async () => {
                              await onQuickCreateMaterial(index, exc.name);
                              setActiveDropdownIndex(null);
                            }}
              ...
              <input
                ...
                onChange={(e) => onChangeExcipient(index, 'declaredContent', e.target.value)}
              ...
              <input
                placeholder="ĐVT"
                value={exc.unit}
                onChange={(e) => onChangeExcipient(index, 'unit', e.target.value)}
              ...
              <button
                type="button"
                onClick={() => onRemoveExcipient(index)}
