...
const TCCSGridItem = React.memo(
  ({
    ...
    return (
      <div
        className={`p-4 rounded-xl transition-all duration-200 group relative overflow-hidden bg-surface border border-border shadow-xs hover:border-emerald-500/30 flex flex-col gap-3 ${isExpanded ? 'md:col-span-2 xl:col-span-3 ring-1 ring-emerald-500/30' : ''}`}
      >
        ...
        <div className="flex items-start justify-between gap-2 relative z-10">
          ...
          {tccs.isActive ? (
            ...
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium bg-surface-2 text-ink-muted border border-border">
              Hết hiệu lực
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 pt-1">
          ...
          <div className="flex flex-col min-w-0">
            <h3 className="font-semibold text-ink text-base leading-tight group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors truncate">
              {tccs.code}
            </h3>
...
const TCCSExpandedStructure = ({ tccs }: { tccs: any }) => {
  ...
  const micro = safety.filter((c: any) => {
    ...
    return (
      c.category === 'micro' || (!c.category && !HEAVY_METAL_KEYWORDS.some((kw) => nl.includes(kw)))
    );
  });
  const metal = safety.filter((c: any) => {
    ...
    const nl = (c.name || '').toLowerCase();
    return (
      c.category === 'metal' || (!c.category && HEAVY_METAL_KEYWORDS.some((kw) => nl.includes(kw)))
    );
  });
  ...
  const CriteriaTable = ({
    ...
    criteria.length > 0 ? (
      <div>
        <h5
          className={`text-xs font-semibold uppercase tracking-wider flex items-center gap-1.5 mb-2 ${color}`}
        >
        ...
        <div className="bg-surface rounded-lg border border-border overflow-hidden mb-3 shadow-xs">
          <table className="w-full text-left text-xs">
            <thead className="bg-surface-2 border-b border-border text-[11px] font-semibold text-ink-muted uppercase">
              <tr>
                <th className="px-3.5 py-2">Chỉ tiêu</th>
                <th className="px-3.5 py-2">Mức quy định</th>
              ...
              {criteria.map((c: any, i: number) =>
                c ? (
                  <tr key={i} className="hover:bg-surface-2 transition-colors">
                    ...
                    <td className="px-3.5 py-2 font-semibold text-emerald-600 dark:text-emerald-400 font-mono">
                      {renderReq(c)}
                    </td>
  ...
  return (
    <div className="px-5 pb-5 pt-3 border-t border-border animate-in slide-in-from-top-4 duration-300 space-y-2 bg-surface-2/40">
      <CriteriaTable
        title="Chỉ tiêu Lý hóa & Cảm quan"
        criteria={ensureArray(tccs?.mainQualityCriteria)}
      ...
      />
      {ensureArray(tccs?.mainQualityCriteria).length === 0 &&
        micro.length === 0 &&
        metal.length === 0 && (
          <p className="text-center text-ink-muted italic text-xs py-3">
            Chưa có chỉ tiêu nào được khai báo.
          </p>
        )}
    </div>
  );
...
const TCCSListItem = React.memo(
  ({ tccs, product, onView, onClone, onEdit, onDelete, isAdmin }: any) => (
...
const TCCSList: React.FC = () => {
  ...
  const handleDeleteTCCS = useCallback(
    async (tccs: TCCS) => {
      ...
      if (isUsed) {
        notify({
          type: 'WARNING',
          title: 'Không thể xóa',
          message: 'TCCS này đang được sử dụng bởi một hoặc nhiều Lô sản xuất.',
        });
        return;
      }
    ...
    if (crud.selectedItem) {
      try {
        ...
        notify({
          type: 'SUCCESS',
          title: 'Đã xóa',
          message: `Đã xóa TCCS ${crud.selectedItem!.code}`,
        });
      } catch (error) {
  ...
  const filteredTCCS = useMemo(() => {
    ...
    return sourceList
      .filter((t) => {
        ...
        const matchesStatus =
          filterStatus === 'ALL' ? true : filterStatus === 'ACTIVE' ? t.isActive : !t.isActive;
        const issueDate = new Date(t.issueDate);
        const matchesYear =
          filterYear === 'ALL' || issueDate.getFullYear().toString() === filterYear;
        const matchesMonth =
          filterMonth === 'ALL' || (issueDate.getMonth() + 1).toString() === filterMonth;
  ...
  const paginatedTCCS = filteredTCCS.slice(
  ...
  const renderComparisonRow = (label: string, val1: any, val2: any) => {
    ...
    return (
      <tr className="border-b border-border last:border-none hover:bg-surface-2">
        ...
        <td
          className={`py-2.5 px-4 text-xs font-semibold ${isDiff ? 'text-emerald-600 bg-emerald-50/50 dark:bg-emerald-950/20' : 'text-ink'}`}
        >
          {val2 || '-'}
        </td>
      </tr>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <DSFilterBar>
        <DSSearchInput
          placeholder="Tìm theo mã TCCS hoặc tên sản phẩm..."
          value={searchTerm}
        ...
        >
          ...
          {Array.from(new Set(tccsList.map((t) => new Date(t.issueDate).getFullYear())))
      ...
      >
        <div className="flex justify-between items-center mb-4 p-3 bg-surface-2 rounded-xl border border-border">
          <p className="text-xs font-medium text-ink flex items-center gap-2">
            <InformationCircleIcon className="w-4 h-4 text-emerald-600" /> Chọn 2 phiên bản để so
            sánh
          </p>
          <button
        ...
        <div className="space-y-3 max-h-[50vh] overflow-y-auto custom-scrollbar pr-2">
          {historyVersions.length > 0 ? (
            <div className="relative border-l-2 border-border ml-3 space-y-4 py-2">
              {historyVersions.map((ver, idx) => (
                <div key={ver.id} className="relative pl-6">
                  <div
                    className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full border-2 border-surface ${idx === 0 ? 'bg-emerald-600' : 'bg-surface-3'}`}
                  />
                  <div className="bg-surface-2 p-3.5 rounded-xl border border-border hover:bg-surface hover:shadow-xs transition-all">
                    <div className="flex justify-between items-start mb-2 gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <h4
                          className={`font-semibold text-sm ${idx === 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-ink'}`}
                        >
                          {ver.code}
                        </h4>
                        <p className="text-xs text-ink-muted">
                          Ban hành: {formatDateStandard(ver.issueDate)}
                        </p>
                      </div>
                      {idx === 0 && (
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400">
                          Hiện hành
                        </span>
                      )}
                    </div>
                    <button
                      ...
                      className="w-full mt-2 py-1.5 bg-surface border border-border rounded-lg text-xs font-medium text-ink-soft hover:text-emerald-600 hover:border-emerald-300 transition-all"
                    >
            ...
            <p className="text-center text-ink-muted text-xs py-4">
              Chưa có dữ liệu lịch sử cho sản phẩm này.
            </p>
          )}
        </div>
      </Modal>
          ...
          <div className="overflow-x-auto max-h-[70vh] custom-scrollbar">
            <div className="grid grid-cols-2 gap-4 mb-4 sticky top-0 bg-surface z-10 pb-3 border-b border-border">
              <div className="p-3.5 bg-surface-2 rounded-xl border border-border">
                ...
                <p className="text-xs text-ink-muted">
                  {formatDateStandard(compareVersions[0].issueDate)}
                </p>
              </div>
              <div className="p-3.5 bg-emerald-50/50 dark:bg-emerald-950/20 rounded-xl border border-emerald-200 dark:border-emerald-800/40 relative">
                ...
                <h4 className="font-semibold text-emerald-700 dark:text-emerald-300 text-sm">
                  {compareVersions[1].code}
                </h4>
                <p className="text-xs text-emerald-600/80 dark:text-emerald-400/80">
                  {formatDateStandard(compareVersions[1].issueDate)}
                </p>
              </div>
            </div>
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-border">
                <tr>
                  <td
                    colSpan={3}
                    className="py-2 px-4 bg-surface-2/60 text-xs font-semibold text-ink-muted uppercase tracking-wider"
                  >
                    Chỉ tiêu chất lượng
                  </td>
                </tr>
                {Array.from(
                  new Set([
                    ...(compareVersions[0].mainQualityCriteria || [])
                      .filter((c) => c?.name)
                      .map((c) => c.name),
                    ...(compareVersions[0].safetyCriteria || [])
                      .filter((c) => c?.name)
                      .map((c) => c.name),
                    ...(compareVersions[1].mainQualityCriteria || [])
                      .filter((c) => c?.name)
                      .map((c) => c.name),
                    ...(compareVersions[1].safetyCriteria || [])
                      .filter((c) => c?.name)
                      .map((c) => c.name),
                  ])
                )
            ...
            <div className="mt-4 flex justify-end">
              <button
                ...
                className="px-4 py-2 bg-surface-2 hover:bg-surface-3 text-ink rounded-lg text-xs font-medium transition-colors border border-border"
              >
                Đóng
              </button>
            </div>
