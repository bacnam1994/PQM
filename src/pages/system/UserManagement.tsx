...
import {
  ...
  UserData,
  UserRole,
  UserAuditLogEntry as AuditLogEntry,
...
const UserManagement: React.FC = () => {
  ...
  useEffect(() => {
    if (!isLogOpen) return;
    const unsub = userService.subscribeUserAuditLogs((loadedLogs) => {
      setLogs(loadedLogs);
    }, 50);
    return () => unsub();
  }, [isLogOpen]);
  ...
  const handleConfirmRoleChange = () => {
    ...
    setConfirmMessage(
      `Bạn có chắc chắn muốn thay đổi vai trò của tài khoản "${selectedUserForRole.email}" từ ${selectedUserForRole.role} sang ${targetRole}?`
    );
    setConfirmButtonColor('bg-indigo-600 hover:bg-indigo-700');
    setConfirmAction(
      () => () => executeRoleChange(selectedUserForRole.uid, selectedUserForRole.role, targetRole)
    );
  ...
  const handleDeleteUser = (user: UserData) => {
    ...
    setConfirmMessage(
      `CẢNH BÁO: Bạn đang thực hiện xóa tài khoản "${user.email}". Người dùng sẽ không thể đăng nhập hoặc truy cập dữ liệu nữa. Hành động này sẽ được ghi vết vào Audit Trail.`
    );
    setConfirmButtonColor('bg-rose-600 hover:bg-rose-700');
    setConfirmAction(() => async () => {
      try {
        await userService.deleteUser(user.uid, user.email, currentUser?.email || 'System Admin');
    ...
    <div className="space-y-6 animate-in fade-in duration-200 pb-20">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-ink flex items-center gap-2.5">
            <UserGroupIcon className="text-emerald-600 dark:text-emerald-400 w-6 h-6" /> Quản trị
            Người dùng & Phân quyền
          </h1>
          <p className="text-ink-muted mt-1 text-xs">
            Quản trị viên có toàn quyền cấp phát, điều chuyển 8 vai trò nghiệp vụ chuẩn GMP và quản
        ...
        >
          <ClockIcon className="w-4 h-4 text-emerald-600 dark:text-emerald-400" /> Lịch sử phân
          quyền (Audit Trail)
        </button>
      </div>
      ...
      <DSTable>
        ...
        <tbody className="divide-y divide-border">
          {loading ? (
            <tr>
              <td colSpan={4} className="p-8 text-center text-ink-muted text-sm font-medium">
              ...
              return (
                <tr key={u.uid} className="hover:bg-surface-2/50 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-center">
                    <span
                      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium border shadow-2xs ${roleDef.badgeBg}`}
                    >
      ...
      >
        <div className="space-y-4">
          <p className="text-xs text-ink-muted">
            Chọn vai trò phù hợp cho người dùng. Thẩm quyền sẽ có hiệu lực ngay lập tức trong toàn
            hệ thống.
          </p>
          ...
          {logs.length === 0 ? (
            ...
            <div className="space-y-3">
              {logs.map((log) => (
                <div
                  key={log.id}
                  className="bg-surface-2/60 p-3 rounded-xl border border-border text-xs"
                >
                  ...
                  <div className="text-ink-muted">
                    {log.action === 'DELETE_USER' ? (
                      ...
                      <>
                        Đã chuyển vai trò của{' '}
                        <span className="font-medium text-emerald-600 dark:text-emerald-400">
                          {log.targetEmail}
                        </span>{' '}
                        từ{' '}
                        <span className="font-mono bg-surface text-ink px-1.5 py-0.5 rounded text-[10px] border border-border">
                          {log.oldRole}
                        </span>{' '}
                        sang{' '}
                        <span className="font-mono bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 px-1.5 py-0.5 rounded text-[10px] font-medium border border-indigo-500/20">
                          {log.newRole}
                        </span>
                      </>
                    )}
