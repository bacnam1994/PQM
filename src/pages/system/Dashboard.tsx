    ...
    <div className="flex flex-col items-center justify-center gap-2">
      ...
      <div className="relative w-40 h-20 overflow-hidden">
        <svg className="w-40 h-40 transform -rotate-180" viewBox="0 0 100 100">
          ...
          {/* Vùng vàng: 50-75% */}
          <circle cx="50" cy="50" r="40" fill="none" stroke="#f59e0b" strokeWidth="7"
            strokeDasharray="31.4 126" strokeDashoffset="-94.2" />
          {/* Vùng xanh: 75-100% */}
          <circle cx="50" cy="50" r="40" fill="none" stroke="#10b981" strokeWidth="7"
      ...
      <div className="text-center -mt-0.5">
        <span className="text-2xl font-bold tracking-tight tabular-nums text-ink">{passRate}%</span>
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
        ...
        <div className="space-y-3.5 max-w-xl z-10">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-ink-muted mt-1 leading-relaxed">
              <strong className="text-ink font-semibold">{stats.totalBatches}</strong> lô đang được theo dõi trong hệ thống. <strong className="text-amber-600 dark:text-amber-400 font-semibold">{alerts.length}</strong> chỉ tiêu chất lượng cần chú ý thẩm định.
            </p>
          </div>
          ...
          <div className="flex items-center gap-5 pt-0.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-ink tabular-nums">{stats.releasedBatches}</span>
              <span className="text-xs text-ink-muted">Đã duyệt</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-amber-500"></span>
              <span className="text-sm font-semibold text-ink tabular-nums">{stats.testingBatches}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-surface p-4 sm:p-5 rounded-xl border border-border shadow-xs hover:border-emerald-500/30 transition-colors flex flex-col justify-between">
          ...
          <div>
            <div className="text-2xl font-bold text-ink tracking-tight tabular-nums">{stats.totalBatches}</div>
        ...
        <div className="bg-surface p-4 sm:p-5 rounded-xl border border-border shadow-xs hover:border-emerald-500/30 transition-colors flex flex-col justify-between">
          ...
          <div>
            <div className="text-2xl font-bold text-ink tracking-tight tabular-nums">{passRate}%</div>
            <div className="text-xs font-medium text-ink-muted mt-0.5">Tỷ lệ đạt chỉ tiêu kiểm nghiệm</div>
          </div>
        </div>
        ...
        <div className="bg-surface p-4 sm:p-5 rounded-xl border border-border shadow-xs hover:border-amber-500/30 transition-colors flex flex-col justify-between">
          ...
          <div>
            <div className="text-2xl font-bold text-ink tracking-tight tabular-nums">{stats.testingBatches}</div>
            <div className="text-xs font-medium text-ink-muted mt-0.5">Lô đang kiểm nghiệm tại Lab</div>
          </div>
        </div>
        ...
        <div className="bg-surface p-4 sm:p-5 rounded-xl border border-border shadow-xs hover:border-rose-500/30 transition-colors flex flex-col justify-between">
          ...
          <div>
            <div className="text-2xl font-bold text-ink tracking-tight tabular-nums">{alerts.length}</div>
            <div className="text-xs font-medium text-ink-muted mt-0.5">Cảnh báo chất lượng phát hiện</div>
          </div>
        </div>
      </div>
      ...
      <div className="p-5 sm:p-6 rounded-xl bg-surface border border-border shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3.5 border-b border-border">
          <div>
            <h2 className="text-sm sm:text-base font-semibold tracking-tight text-ink">Chuỗi xử lý lô — Từ nguyên liệu đến xuất kho</h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3.5">
          ...
          <div className="p-3.5 rounded-xl bg-surface-2/60 border border-border text-center flex flex-col items-center hover:bg-surface-2 transition-colors">
            ...
            <div className="text-lg font-bold text-ink tabular-nums">{pipelineStages.production}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 p-5 rounded-xl bg-surface border border-border shadow-xs space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-border">
            ...
            <Link to="/batches" className="text-xs font-medium text-emerald-600 dark:text-emerald-400 hover:underline">
              Xem tất cả &rarr;
            </Link>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-3">
                        <div className="font-medium text-ink">{product?.name || 'Sản phẩm không rõ'}</div>
                        <div className="text-[11px] font-mono text-ink-muted">{product?.code || '---'}</div>
                      </td>
        ...
        <div className="space-y-4">
          ...
          <div className="p-5 rounded-xl bg-surface border border-border shadow-xs space-y-3">
            <div className="flex items-center justify-between pb-3 border-b border-border">
              ...
              <Link to="/alerts" className="text-xs font-medium text-emerald-600 dark:text-emerald-400 hover:underline">
                Tất cả &rarr;
              </Link>
            </div>
                ...
                return (
                  <div key={idx} className="py-2.5 flex items-start gap-2.5">
                    <div className={`p-1.5 rounded-lg shrink-0 mt-0.5 ${
                      ...
                      {isHigh ? <ShieldExclamationIcon className="w-3.5 h-3.5" /> : <ClockIcon className="w-3.5 h-3.5" />}
                    </div>
