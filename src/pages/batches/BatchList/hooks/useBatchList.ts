...
import { BATCH_STATUS, formatDateStandard, parseDateToISO, generateId, normalizeSearch } from '../../../../utils';
...
export function useBatchList() {
  ...
  const filteredBatches = useMemo(() => {
    return hydratedBatches.filter(batch => {
      // Tìm kiếm từ khóa
      ...
      if (sortConfig.key === 'createdAt') {
        return (new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()) * (sortConfig.direction === 'desc' ? 1 : -1);
      }
  ...
  }, [hydratedBatches, debouncedSearchTerm, filterStatus, filterProductId, filterYear, filterMonth, dateRange, sortConfig]);
  ...
  const confirmBatchStatusUpdate = async () => {
    ...
    try {
      await updateBatchStatus(pendingStatusUpdate.batchId, pendingStatusUpdate.status as any, rejectReason);
