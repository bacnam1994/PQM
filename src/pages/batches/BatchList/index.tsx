        ...
        {!isAdvancedFilterOpen && (
          <>
            ...
            <DSSelect value={filterMonth} onChange={(e) => setFilterMonth(e.target.value)} className="w-32">
              ...
              {Array.from({ length: 12 }, (_, i) => i + 1).map(month => <option key={month} value={month.toString()}>Tháng {month}</option>)}
            </DSSelect>
          </>
        )}
        ...
        <div className="bg-surface p-4 rounded-xl border border-border shadow-xs grid grid-cols-1 md:grid-cols-4 gap-4 animate-in slide-in-from-top-2">
          ...
          <div className="space-y-1.5 md:col-span-2">
            <div className="flex justify-between items-center">
              ...
              <button 
                onClick={() => { setDateRange({ from: '', to: '' }); setFilterProductId(''); setFilterStatus('ALL' as any); }} 
