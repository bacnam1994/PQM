  ...
  return (
    <div className="fixed top-4 right-4 z-[110] flex flex-col gap-3 w-full max-w-sm pointer-events-none">
      {toasts.map(t => {
        ...
        switch (t.type) {
          case 'SUCCESS':
            bgClass = 'bg-emerald-50/95 dark:bg-emerald-950/80 border-emerald-200 dark:border-emerald-800 text-ink';
            icon = <CheckCircleIcon className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />;
            break;
          case 'ERROR':
            bgClass = 'bg-rose-50/95 dark:bg-rose-950/80 border-rose-200 dark:border-rose-800 text-ink';
            icon = <ExclamationCircleIcon className="w-5 h-5 text-rose-600 dark:text-rose-400" />;
            break;
          case 'WARNING':
            bgClass = 'bg-amber-50/95 dark:bg-amber-950/80 border-amber-200 dark:border-amber-800 text-ink';
            icon = <ExclamationTriangleIcon className="w-5 h-5 text-amber-600 dark:text-amber-400" />;
            break;
        }

        return (
          <div key={t.id} className={`pointer-events-auto flex items-start gap-3 p-4 rounded-2xl border shadow-xl backdrop-blur-md animate-in slide-in-from-right duration-300 ${bgClass}`}>
            ...
            <button onClick={() => removeToast(t.id)} className="text-ink-muted hover:text-ink transition-colors">
