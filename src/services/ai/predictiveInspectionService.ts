...
const countTotalSamples = (ctx: PredictiveContext) => ctx.productTestResults.length;
...
const calcLabPerformanceFactor = (
  ...
  return {
    score: passRate,
    label:
      passRate >= 90 ? '✅ Lab uy tín' : passRate >= 70 ? '⚠️ Cần chú ý' : '🔴 Tỷ lệ thất bại cao',
...
const calcSeasonalFactor = (
  ...
  if (!ctx.batch.mfgDate) {
    return {
      ...
      detail: 'Không có ngày sản xuất để phân tích mùa vụ.',
    };
  }
  ...
  return {
    ...
    label:
      passRate >= 85
        ? `📅 Tháng ${batchMonth} tốt`
        : passRate >= 65
          ? `⚠️ Tháng ${batchMonth} rủi ro`
          : `🔴 Tháng ${batchMonth} nguy hiểm`,
...
const calcTrendDriftFactor = (
  ...
  if (recentResults.length < 4) {
    return {
      score: 75,
      label: 'Dữ liệu xu hướng ít',
      detail: `Chỉ có ${recentResults.length} phiếu gần nhất (cần ≥ 4).`,
    };
  }
...
const calcHistoricalSimilarityFactor = (
  ctx: PredictiveContext
): { score: number; label: string; detail: string } => {
  ...
  if (similarBatches.length === 0) {
    return {
      score: 70,
      label: 'Không có lô tương đồng',
      detail: 'Không tìm thấy lô lịch sử tương đồng để so sánh.',
    };
  }
  ...
  if (similarResults.length === 0) {
    return {
      score: 70,
      label: 'Chưa có kết quả lô tương đồng',
      detail: 'Các lô tương đồng chưa có phiếu kiểm nghiệm.',
    };
  }
  ...
  return {
    ...
    label:
      passRate >= 80
        ? '✅ Lô tương tự đạt tốt'
        : passRate >= 60
          ? '⚠️ Lô tương tự có lô fail'
          : '🔴 Lô tương tự fail nhiều',
...
const analyzeHighRiskCriteria = (ctx: PredictiveContext, tccs: any): CriterionRiskFactor[] => {
  if (!tccs) return [];

  const mainCriteria = [...(tccs.mainQualityCriteria || []), ...(tccs.safetyCriteria || [])].filter(
    (c: any) => c && c.name && c.type === 'NUMBER'
  );
    ...
    .map((criterion: any) => {
      ...
      recentResults.forEach((r) => {
        const entry = (r.results || []).find(
          (res: any) => res.criteriaName?.toLowerCase() === criterion.name?.toLowerCase()
        );
        if (!entry) return;
        const val = parseFloat(String(entry.value).replace(',', '.'));
        if (!isNaN(val)) values.push(val);
        if (normalizeCriterionPassStatus(entry.isPass) === false) failCount++;
      });
      ...
      if (values.length >= 3) {
        ...
        const std = Math.sqrt(
          deltas.map((d) => (d - avgDelta) ** 2).reduce((a, b) => a + b, 0) / deltas.length
        );
      ...
      if (criterion.max !== undefined && criterion.min !== undefined) {
        ...
        if (range > 0) {
          ...
          if (normalizedDist > 0.85)
            riskReason = `Giá trị gần đây (${latest}) tiệm cận ngưỡng giới hạn`;
      ...
      const riskLevel: CriterionRiskFactor['riskLevel'] =
        riskScore > 80 ? 'HIGH' : riskScore > 55 ? 'MEDIUM' : 'LOW';
...
export const predictInspectionOutcome = (ctx: PredictiveContext): PredictiveInspectionReport => {
  ...
  const weightedScore =
    ...
    trendFactor.score * 0.3 +
    seasonalFactor.score * 0.2 +
  ...
  if (passProb < 60)
    recommendations.push('🔴 Tăng cường kiểm tra IPC và tất cả chỉ tiêu trong quá trình');
  if (labFactor.score < 70)
    recommendations.push(
      `⚠️ Cân nhắc kiểm tra chéo với phòng lab khác ngoài "${ctx.batch.labName || 'hiện tại'}"`
    );
  if (seasonalFactor.score < 70)
    recommendations.push('📅 Chú ý điều kiện môi trường đặc thù của tháng này');
  if (highRiskCriteria.length > 0) {
    const top = highRiskCriteria[0];
    recommendations.push(
      `🎯 Ưu tiên kiểm tra chặt chỉ tiêu "${top.criteriaName}" (rủi ro ${top.riskLevel})`
    );
  }
  if (trendFactor.score < 60)
    recommendations.push('📉 Điều tra nguyên nhân xu hướng giảm chất lượng gần đây');
  if (recommendations.length === 0)
    recommendations.push('✅ Tiến hành kiểm nghiệm theo kế hoạch bình thường');
  ...
  return {
    ...
    summary,
    disclaimer:
      'Đây là dự báo dựa trên dữ liệu lịch sử, không phải kết quả kiểm nghiệm chính thức. Quyết định xuất xưởng phải dựa trên kết quả kiểm nghiệm thực tế theo quy định GMP.',
  };
};
...
export const calculateStabilityKinetics = (
  ...
  // Sắp xếp theo ngày
  const sorted = [...timeSeriesPoints].sort(
  ...
  const isEarlyExpiryRisk =
    (monthsToMinLimit !== null && monthsToMinLimit < shelfLifeMonths) ||
    predictedExpiryValue < minLimit;
  const status: StabilityKineticsResult['status'] = isEarlyExpiryRisk
    ? 'RAPID_DECAY'
    : k > 0.01
      ? 'MODERATE_DECAY'
      : 'STABLE';
...
export const predictBatchRiskBeforeTesting = (
  ...
  const tccs =
    appContext.tccsList.find((t) => t.id === batch.tccsId) ||
