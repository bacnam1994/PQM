  ...
  static evaluate(
    ...
    if (user.isAdmin || user.role === 'ADMIN') {
      ...
      if (resourcePath.startsWith('audit_logs') && (action === 'UPDATE' || action === 'DELETE')) {
        return { allowed: false, reason: 'ALCOA+ Violation: Nhật ký kiểm toán là bất biến, không thể sửa đổi hoặc xóa.' };
      }
      return { allowed: true };
    }
    ...
    if (rootCollection === 'users') {
      ...
      if (action === 'UPDATE' || action === 'CREATE') {
        if (payload?.role || payload?.isAdmin) {
          return { allowed: false, reason: 'Không có quyền tự cấp quyền (Role Escalation Prevention).' };
        }
        return { allowed: true };
      }
    ...
    if (rootCollection === 'products' || rootCollection === 'raw_materials' || rootCollection === 'product_formulas') {
      if (action === 'READ') return { allowed: true };
      if (user.role === 'QA') return { allowed: true };
      return { allowed: false, reason: 'Chỉ QA hoặc Admin mới có quyền sửa đổi dữ liệu sản phẩm / danh mục.' };
    }
    ...
    if (rootCollection === 'tccs') {
      if (action === 'READ') return { allowed: true };
      if (user.role === 'QA') return { allowed: true };
      return { allowed: false, reason: 'Chỉ QA hoặc Admin mới có quyền cập nhật Tiêu chuẩn cơ sở.' };
    }
    ...
    if (rootCollection === 'batches') {
      ...
      if (payload?.status === 'RELEASED' || payload?.status === 'REJECTED') {
        if (user.role !== 'QA') {
          return { allowed: false, reason: 'Chỉ QA mới có thẩm quyền Phê duyệt xuất xưởng (RELEASED) hoặc Từ chối (REJECTED) lô sản xuất.' };
        }
      }
      // Lô đã RELEASED: không cho user bình thường chỉnh sửa
      if (currentData?.status === 'RELEASED' && user.role !== 'QA') {
        return { allowed: false, reason: 'Lô sản xuất đã xuất xưởng (RELEASED) bị khóa, không thể chỉnh sửa.' };
      }
      return { allowed: true };
    }
    ...
    if (rootCollection === 'testResults') {
      if (action === 'READ') return { allowed: true };
      // Nếu phiếu đã APPROVED hoặc LOCKED: chặn LAB/QC sửa đổi
      if ((currentData?.overallStatus === 'APPROVED' || currentData?.status === 'LOCKED') && user.role !== 'QA') {
        return { allowed: false, reason: 'Phiếu kiểm nghiệm đã được khóa/duyệt, không thể sửa đổi.' };
      }
      return { allowed: true };
    }
  ...
  static validateStorageUpload(
    ...
    if (folder === 'test_attachments') {
      ...
      if (!allowedTypes.includes(file.type)) {
        return { allowed: false, reason: 'Tệp đính kèm chỉ chấp nhận định dạng PDF hoặc hình ảnh (JPEG, PNG, WEBP).' };
      }
    ...
    if (folder === 'product_images') {
      if (user.role !== 'QA' && !user.isAdmin) {
        return { allowed: false, reason: 'Chỉ QA hoặc Admin mới có quyền cập nhật hình ảnh sản phẩm.' };
      }
