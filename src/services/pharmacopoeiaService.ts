...
/**
 * Tải danh mục tiêu chuẩn dược điển từ Repository
 */
export const fetchPharmacopoeiaStandards = async (): Promise<PharmacopoeiaStandard[]> => {
  try {
    const list = await firebasePharmacopoeiaRepository.findAll();
    if (list && list.length > 0) {
      const sanitized = list.map((item) => ({
        ...item,
        keywords: Array.isArray(item.keywords) ? item.keywords : [],
      }));
      inMemoryStandards = sanitized;
      isLoadedFromRTDB = true;
      return sanitized;
    } else {
      // Nếu repository rỗng, nạp sẵn bộ mặc định
      await seedDefaultPharmacopoeiaStandards();
      return inMemoryStandards;
    }
  } catch (error) {
    console.warn('[Pharmacopoeia] Lỗi tải từ Repository, sử dụng fallback cục bộ:', error);
    return inMemoryStandards;
  }
};
...
/**
 * Nạp bộ dữ liệu mặc định lên Repository
 */
export const seedDefaultPharmacopoeiaStandards = async (): Promise<void> => {
  try {
    await firebasePharmacopoeiaRepository.seedDefaults(DEFAULT_PHARMACOPOEIA_DB);
    inMemoryStandards = [...DEFAULT_PHARMACOPOEIA_DB];
    isLoadedFromRTDB = true;
  } catch (error) {
    console.error('[Pharmacopoeia] Lỗi nạp dữ liệu mẫu lên Repository:', error);
    throw error;
  }
};
...
export const savePharmacopoeiaStandard = async (
  ...
  await firebasePharmacopoeiaRepository.save(payload);
...
/**
 * Xóa một tiêu chuẩn khỏi Repository
 */
export const deletePharmacopoeiaStandard = async (id: string): Promise<void> => {
  await firebasePharmacopoeiaRepository.delete(id);
