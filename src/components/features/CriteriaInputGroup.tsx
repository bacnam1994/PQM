...
const CriteriaInputRow = memo(({ c, criteriaName, currentVal, history, onChange, isAiFilled }: CriteriaInputRowProps) => {
  ...
  // Xác định border và background dựa trên trạng thái
  ...
  return (
    <div className={`flex flex-col gap-2 p-4 rounded-xl border-2 transition-all ${containerClass}`}>
      <div className="flex justify-between items-start gap-2">
        <div className="flex-1 min-w-0">
          <p className={`text-[10px] font-black uppercase mb-1 truncate ${
            hasValue ? (isPass ? 'text-emerald-700' : 'text-red-700') : 'text-slate-800'
          }`}>{criteriaName}</p>
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          {hasValue && (
            <div className={`px-2 py-1 rounded text-[9px] font-black uppercase ${
              isPass ? 'bg-emerald-200 text-emerald-700' : 'bg-red-200 text-red-700'
            }`}>
              {isPass ? 'ĐẠT' : 'K.ĐẠT'}
            </div>
          )}
