...
const BatchCriteriaHistory: React.FC<BatchCriteriaHistoryProps> = ({ batchId }) => {
  ...
  const allTestResults =
    (useAppStore((state) => (state as any).allTestResults) as TestResult[]) || [];
  ...
  useEffect(() => {
    ...
    const fetchHistoryForBatch = async () => {
      ...
      try {
        const finalResults = await fetchTestResultsByBatchId(batchId);
        if (!isMounted) return;
    ...
    if (!activeTCCS) {
      ...
      if (productTccs.length > 0) {
        if (batch.mfgDate) {
          ...
          activeTCCS =
            productTccs.find((t) => new Date(t.issueDate).getTime() <= mfgTime) ||
            productTccs[productTccs.length - 1];
        } else {
    ...
    try {
      ...
      const groupedData = tccsCriteria.map((criterion) => {
        ...
        batchResults.forEach((res) => {
          ...
          const match = ensureArray(res.results).find(
            (r) => r && resolver.isMatch(r.criteriaName, criterion.name)
          );
          if (match) {
            history.push({
              ...match,
      ...
      batchResults.forEach((res) => {
        ensureArray(res.results).forEach((r) => {
          ...
          const isMatchedInTCCS = tccsCriteria.some(
            (c) => c && resolver.isMatch(r.criteriaName, c.name)
          );
          if (!isMatchedInTCCS) {
  ...
  if (!tccs)
    return (
      <div className="p-4 text-center text-ink-muted italic">
        Chưa xác định được TCCS cho lô này.
      </div>
    );
  ...
  const renderRequirement = (c: any) => {
    ...
    return c.expectedText
      ? `${c.expectedText} ${c.unit && !String(c.expectedText).includes(c.unit) ? c.unit : ''}`.trim()
      : '';
  };
  ...
  const renderHistoryRow = (entry: HistoryEntry, idx: number, tccsUnit?: string) => {
    ...
    return (
      <div
        key={idx}
        className="flex items-center gap-3 text-xs bg-surface border border-border p-2.5 rounded-lg shadow-sm mb-1 last:mb-0"
      >
        <div
          className={`shrink-0 ${entry.isPass ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-500'}`}
          title={entry.isPass ? 'Đạt' : 'Không đạt'}
        >
          {entry.isPass ? (
        ...
        <div className="w-28 flex flex-col items-end">
          <div className="flex items-baseline gap-1">
            ...
            <span
              className="text-[10px] text-ink-muted font-medium truncate max-w-[40px]"
              title={displayUnit}
            >
              {displayUnit}
            </span>
        ...
        <div className="flex-1 text-ink-muted text-[10px] flex items-center gap-1.5">
          <span
            className="font-bold text-emerald-700 dark:text-emerald-300 truncate max-w-[120px]"
            title={entry.labName}
          >
            {entry.labName}
          </span>
        ...
        {(entry as any).limit && (
          <div className="text-[9px] text-ink-muted hidden sm:block">
            (GH: {(entry as any).limit})
          </div>
        )}
      </div>
    );
          ...
          <tbody className="divide-y divide-border">
              ...
              <tr key={idx} className="hover:bg-surface-2/50 transition-colors">
                <td className="py-2.5 px-4 align-top">
                  ...
                  <p className="text-[10px] text-ink-muted mt-0.5">
                    {renderRequirement(criterion)}
                  </p>
                  {history.length === 0 && (
                    <span className="inline-block mt-1 px-2 py-0.5 bg-surface-2 text-ink-muted rounded text-[9px] font-bold">
                      Chưa kiểm
                    </span>
                  )}
                </td>
            ...
            {extraData.length > 0 && (
              <tr>
                <td
                  colSpan={2}
                  className="py-2.5 px-4 bg-surface-2 font-black text-ink-muted text-[10px] uppercase tracking-widest border-t border-border"
                >
